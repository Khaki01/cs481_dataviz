...
dist_plot = ff.create_distplot(
    hist_data_total, ["Phone usage"], show_hist=False, show_rug=False, colors=colors
)
    ...
    [
        dbc.Card(
            [
                ...
                dcc.Graph(id="plot", className="graph-style"),
            ],
...
def update_plot(daysdropdown, appsdropdown, goalslider, hoverData):
    ...
    if appsdropdown != "all":
        ...
        trace1 = go.Bar(
            x=days_subset, y=goal_subset, name="goal", marker=dict(opacity=0.7)
        )
        trace2 = go.Bar(
            x=days_subset,
            y=extra_subset,
