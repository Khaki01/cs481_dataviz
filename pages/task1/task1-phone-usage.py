...
density = [
    [150, 120, 130, 140, 140, 130, 120, 110],
    [100, 120, 130, 100, 90, 80, 70, 70],
    [100, 120, 130, 100, 90, 80, 70, 70],
    [100, 120, 130, 100, 90, 80, 70, 70],
]
...
colors = [
    color.hex_to_rgba(col) for col in px.colors.sequential.Plasma[: len(labels) + 1]
]

# GALA
days = ["Mon", "Tue", "Wed", "Thur", "Fri", "Sat", "Sun"]
goal = [2, 1.5, 1.2, 1.0, 0.8, 0.7, 0.65]
insta = [2, 1.5, 1.2, 1.0, 0.8, 0.7, 0.65]
tiktok = [1, 1.2, 1.5, 1.4, 0.2, 0.3, 1.0]
play = [0.1, 0.3, 0.4, 0.6, 1.5, 0, 0.35]
                ...
                html.Div(
                    [
                        ...
                        html.Div(
                            [
                                html.Label(
                                    "End Goal(hour):", style={"marginLeft": "20px"}
                                ),
                                dcc.Slider(
                                    id="goalslider",
                                    min=0,
...
def update_graph(clickData):
    ...
    array[pos] = 0.5
    colors_updated = [
        color[:-2] + "0.5)" if i != pos else color for (i, color) in enumerate(colors)
    ]
    pie.update_traces(
...
def update_plot(daysdropdown, appsdropdown, goalslider):
    ...
    if appsdropdown != "all":
        ...
        trace3 = go.Bar(
            x=days_subset, y=app_data, name=appsdropdown, marker=dict(color="orange")
        )
        trace4 = go.Bar(
            x=days_subset,
            y=other_subset,
            name="Other Apps",
            marker=dict(color="rgba(0, 128, 0, 0.2)"),
        )
        ...
        trace2 = go.Bar(
            x=days_subset, y=extra_subset, name="extra", marker=dict(color=colors)
        )
